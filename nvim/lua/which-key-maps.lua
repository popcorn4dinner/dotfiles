...
local files = {
  ...
  d = {
    name = "dotfiles",
    k = {":edit ~/.config/nvim/lua/which-key-maps.lua<cr>", "key maps"},
    v = {":edit ~/.config/nvim/init.vim<cr>", "vim"},
    R = {":Restart<cr>", "reload neovim"}
  },
  D = {":call delete(expand('%')) | bw!<cr>", "delete"},
  f = { function() require('telescope.builtin').file_browser(require('telescope.themes').get_dropdown({cwd = vim.fn.expand('%:p:h')})) end, "find files"},
...
local project = {
  ...
  s = {"<cmd>Telescope live_grep theme=get_dropdown<cr>", "search"},
...
local text = {
  name = "text",
  s = {"<cmd>Telescope spell_suggest theme=get_dropdown<cr>", "spell suggestions"},
